...
for (analysis.version in c("0.1")){
  path.to.html.output <- file.path(path.to.main.src, sprintf("html_outputs_20240215_%s", analysis.version))
  dir.create(path.to.html.output, showWarnings = FALSE, recursive = TRUE)
  
  path.to.RMD.files <- c(file.path(path.to.main.src, "04_correlation_between_V_and_I_samples.Rmd"))
